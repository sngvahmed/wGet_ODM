...
public class WGet {
	...
	public void updateDirectboolSize(int THREAD_COUNT) {
		if (info.multipart()) {
			((DirectMultipart) d).setTHREAD_COUNT(THREAD_COUNT);
		}
	}
	...
	public static File calcName(DownloadInfo info, File target) {
		...
		if (target.isDirectory()) {
			...
			while (targetFile.exists()) {
				targetFile = FileUtils.getFile(target, nameNoExt + " (" + i
						+ ")." + ext);
				i++;
			}
		} else {
	...
	public static String getHtml(final URL source, final HtmlLoader load,
			final AtomicBoolean stop) {
		return getHtml(new DownloadInfo(source), load, stop);
	}

	public static String getHtml(final DownloadInfo source,
			final HtmlLoader load, final AtomicBoolean stop) {
		String html = RetryWrap.wrap(stop, new RetryWrap.WrapReturn<String>() {
			...
			public String download() throws IOException {
				...
				if (info.getReferer() != null)
					conn.setRequestProperty("Referer", info.getReferer()
							.toExternalForm());
